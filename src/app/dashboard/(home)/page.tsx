...
import { Edit, HandCoins, LogIn } from "lucide-react";
        ...
        >
          <div className='bg-gradient-to-b from-black/30 to-gray-950 text-white h-full flex flex-col'>
            ...
            <CardFooter className='flex-col items-start gap-2 text-gray-200 text-sm'>
              ...
              <div className='flex gap-3 items-start hover:text-dark-blue/60 transition ease-linear duration-200 cursor-pointer font-semibold'>
                <HandCoins strokeWidth={1.5} className='size-5 flex-shrink-0' />
                <p>
                  Revenue: ₦{" "}
                  {parseFloat(
                    res?.data?.totalRevenue?.toFixed(2) ?? "0"
                  ).toLocaleString()}
                </p>
              </div>

              <div className='flex gap-3 items-start hover:text-dark-blue/60 transition ease-linear duration-200 cursor-pointer font-semibold'>
                <LogIn strokeWidth={1.5} className='size-5 flex-shrink-0' />
                <p>Checkins: {res?.data?.event?.total_checked_in}</p>
              </div>
            </CardFooter>
          </div>
        </Card>
